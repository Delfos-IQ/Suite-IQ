...
function renderRecommendation(ws, activeW) {
  // Find or create recommendation banner
  ...
  // Don't show if active watch — already wearing something
  if (activeW) { banner.innerHTML = ''; return; }

  // Find the watch least recently used (among automatics primarily, then all)
  const ranked = rankWatchesByNeed(ws);
  if (!ranked.length) { banner.innerHTML = ''; return; }

  const top = ranked[0];
  const w   = top.watch;
  const daysAgo = top.daysSinceLastWorn;

  const urgencyColor = daysAgo === null || daysAgo > 25 ? '#e57373'   // overdue — red
    : daysAgo > 14 ? '#D4AF6A'                                        // approaching — gold
    : '#4CAF50';                                                       // ok — green

  const urgencyText = daysAgo === null
    ? 'Nunca usado — ¡el aceite se seca!'
    : daysAgo > 25
    ? `Hace ${daysAgo} días — necesita movimiento`
    : daysAgo > 14
    ? `Hace ${daysAgo} días — pronto habrá que ponérselo`
    : `Hace ${daysAgo} días — está bien`;

  banner.innerHTML = `
    <div class="recommendation-card" onclick="openDetail('${escHtml(w.id)}')">
      <div class="rec-icon"><i class="ti ti-alarm" style="color:${urgencyColor};font-size:20px;"></i></div>
      <div class="rec-body">
